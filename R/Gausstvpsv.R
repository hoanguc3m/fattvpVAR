#' @export
hybrid_tvpsvfunc <- function(y, p, priors, inits){

  Y0 <- head(y,p)
  shortY <- tail(y, nrow(y) - p)
  t_max <- nrow(shortY)
  K <- ncol(shortY)
  ...
  ## MCMC starts here
  set.seed(NULL)
  sprintf('Starting MCMC for (hybrid) TVP-VAR-SV with ID: %s ', paste(as.character(is_tv), collapse = ' ') )
  start_time <- Sys.time()
  ...
  for (isim in c(1:(samples + burnin))){
    ...
    for (ii in 1:K){
      ...
      if (is_tv[ii] == 1){
        ...
        XiSig <- t(bigXi) %*% sparseMatrix(i = 1:t_max, j = 1:t_max, x = exp(-h[,ii]))
