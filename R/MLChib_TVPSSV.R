...
Chib_fitTVP_SSV <- function(y, K, p, y0 = NULL, priors = NULL, inits = NULL,
  ...
  for (j in c(1:samples)){
    ...
    E = U / sqrt_w
    # sample latent h
    aux <- sample_h_mod(ytilde = t(E), sigma_h = Sigh, h0_mean = h0,
...
#' @export
ChibLLP_TVP_SSV <- function(Chain, ndraws = 1000, numCores = NULL){
  ...
  Nu_med <- apply(Chain$store_nu, MARGIN = 2, FUN = mean)
  ...
  sum_log_prior <- sum(dgamma(Sigmah_med, shape = 0.5, rate = 0.5 / priors$hyper_h, log = T),
                       ...
                       dgamma(Nu_med, shape = priors$nu_gam_a, rate = priors$nu_gam_b, log = T) )
  dpriorABS <- rep(0, K)
  for (ii in c(1:K) ){
